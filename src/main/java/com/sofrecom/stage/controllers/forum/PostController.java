...
public class PostController {
    ...
    @RequestMapping(value = "/addVideoFile/{userId}" ,method = RequestMethod.POST )
    public Post addVideoPost(@PathVariable("userId") Long userId,@RequestParam("file") MultipartFile file,@RequestParam("details") String post) throws IOException {
        /*if(file == null){
            throw new NullPointerException("We cannot save null ! ");
        }*/
        Post pDetails = new ObjectMapper().readValue(post,Post.class);
        System.out.println(pDetails);
        Post p = postService.saveVideoFile(userId,file,pDetails);
        return p;
    }
